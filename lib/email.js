...
export async function sendEmail({ to, subject, html }) {
    if (!process.env.EMAIL_USER || !process.env.EMAIL_PASSWORD) {
        console.error('❌ ERROR: EMAIL_USER o EMAIL_PASSWORD no configurados en variables de entorno.');
        return { success: false, error: 'Credenciales de email faltantes' };
    }
...
export async function sendEnrollmentEmails(studentData) {
    const { name, email, workshopTitle, selectedSchedules, dni, phone, tutorName } = studentData;
